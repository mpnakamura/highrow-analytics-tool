...
export default function TradeAnalysis({ data }: TradeAnalysisProps) {
  const [activeTab, setActiveTab] = useState<'overview' | 'hourly' | 'date' | 'strategy'>('overview');
    ...
    <div className="w-full">
      <div className="card mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-[var(--muted)] text-[var(--muted-foreground)] p-4 rounded">
            ...
            <p className="text-2xl font-bold text-[var(--foreground)]">{summary.total}回</p>
          </div>
          <div className="bg-[var(--muted)] text-[var(--muted-foreground)] p-4 rounded">
            <p className="text-[var(--muted-foreground)]">勝ち</p>
            <p className="text-2xl font-bold text-[var(--success)]">{summary.wins}回</p>
          </div>
          <div className="bg-[var(--muted)] text-[var(--muted-foreground)] p-4 rounded">
            <p className="text-[var(--muted-foreground)]">負け</p>
            <p className="text-2xl font-bold text-[var(--danger)]">{summary.losses}回</p>
          </div>
          <div className="bg-[var(--muted)] text-[var(--muted-foreground)] p-4 rounded">
            <p className="text-[var(--muted-foreground)]">勝率</p>
            <p className="text-2xl font-bold text-[var(--foreground)]">{Math.round(summary.winRate * 100) / 100}%</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        <div className="flex border-b border-[var(--border)]">
          ...
          <button 
            className={`tab-button ${activeTab === 'hourly' ? 'tab-button-active' : ''}`}
            onClick={() => setActiveTab('hourly')}
          >
            時間帯別
          </button>
          <button 
            className={`tab-button ${activeTab === 'date' ? 'tab-button-active' : ''}`}
        ...
        {activeTab === 'date' && (
          <div className="grid grid-cols-1 gap-6 p-4">
            ...
            <div className="card">
              <h3 className="text-xl font-bold mb-4">日付と時間帯の組み合わせ勝率</h3>
              <div className="h-96">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
          ...
          <div className="card m-4">
            ...
            <div className="mb-6">
              ...
              <ul className="list-disc pl-6">
                {strategy.topHours.map((hour, idx) => (
                  <li key={idx}>
                    {hour.hour}: <span className="text-[var(--success)]">{Math.round(hour.winRate * 100) / 100}%</span> ({hour.wins}/{hour.total})
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="mb-6">
              ...
              <ul className="list-disc pl-6">
                {strategy.worstHours.map((hour, idx) => (
                  <li key={idx}>
                    {hour.hour}: <span className="text-[var(--danger)]">{Math.round(hour.winRate * 100) / 100}%</span> ({hour.wins}/{hour.total})
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="mb-6">
              ...
              <ol className="list-decimal pl-6">
                <li className="mb-2">
                  <span className="font-medium">高勝率の時間帯を重視:</span> 特に
                  {strategy.topHours.map((h, i) => (
                    <span key={i}>
                      {i === 0 ? ' ' : i === strategy.topHours.length - 1 ? ' と ' : '、'}
                ...
                <li className="mb-2">
                  ...
                  {strategy.worstHours.map((h, i) => (
                    <span key={i}>
                      {i === 0 ? ' ' : i === strategy.worstHours.length - 1 ? ' と ' : '、'}
                ...
                <li className="mb-2">
                  ...
                  {highLow[0].winRate > highLow[1].winRate 
                    ? <span> <span className="text-[var(--primary)]">HIGH</span>取引の方が勝率が高い傾向があります。</span> 
                    : <span> <span className="text-[var(--primary)]">LOW</span>取引の方が勝率が高い傾向があります。</span>}
                </li>
              </ol>
            </div>
          </div>
