...
export function analyzeTrades(trades: Trade[]): AnalysisResult {
  ...
  // 重複を除去（取引番号で判断）
  const uniqueBtcTrades = btcTrades.filter(
    (trade, index, self) =>
      index === self.findIndex((t) => t["取引番号"] === trade["取引番号"])
  );

  console.log(
    `元のデータ: ${trades.length}件, BTCデータ: ${btcTrades.length}件, 重複除外後: ${uniqueBtcTrades.length}件`
  );
  ...
  // 時間帯ごとの勝率を計算して配列に変換
  const hourlyData: HourlyStats[] = Object.entries(hourlyStats).map(
    ([hour, stats]) => {
      return {
        hour: `${hour}時台`,
        wins: stats.wins,
  ...
  // 日付ごとの勝率を計算して配列に変換
  const dateData: DateStats[] = Object.entries(dateStats).map(
    ([date, stats]) => {
      return {
        date,
        wins: stats.wins,
  ...
  tradesWithResult.forEach((trade) => {
    ...
    if (trade.result === "勝ち") {
      ...
      monthlyStats[monthKey].profit += Number(
        trade.ペイアウト.replace(/[^0-9.-]+/g, "")
      );
    } else if (trade.result === "負け") {
      monthlyStats[monthKey].losses++;
      monthlyStats[monthKey].profit -= Number(
        trade.購入金額.replace(/[^0-9.-]+/g, "")
      );
    }
  ...
  // 月別分析データの作成
  const monthlyAnalysis = Object.entries(monthlyStats).reduce(
    (acc, [month, stats]) => {
      acc[month] = {
        total: stats.total,
        winRate: (stats.wins / stats.total) * 100,
  ...
  const totalInvestment = tradesWithResult.reduce(
    (sum, trade) => sum + Number(trade.購入金額.replace(/[^0-9.-]+/g, "")),
    0
  );

  const totalPayout = tradesWithResult.reduce(
    (sum, trade) =>
      trade.result === "勝ち"
        ? sum + Number(trade.ペイアウト.replace(/[^0-9.-]+/g, ""))
        : sum,
    0
  );
  ...
  const averageProfit =
    profits.filter((p) => p > 0).reduce((sum, p) => sum + p, 0) / wins || 0;
  const averageLoss =
    profits.filter((p) => p < 0).reduce((sum, p) => sum + p, 0) / losses || 0;
  ...
  return {
    summary: {
      ...
      totalPayout,
      averageAmount:
        tradesWithResult.reduce(
          (sum, trade) =>
            sum + Number(trade.購入金額.replace(/[^0-9.-]+/g, "")),
          0
        ) / total,
    },
    highLow: [
      {
        name: "HIGH",
        wins: highWins,
        losses: highTrades.length - highWins,
        total: highTrades.length,
        winRate: (highWins / highTrades.length) * 100,
      },
      {
        name: "LOW",
        wins: lowWins,
        losses: lowTrades.length - lowWins,
        total: lowTrades.length,
        winRate: (lowWins / lowTrades.length) * 100,
      },
    ],
    hourly: hourlyData,
    dateStats: dateData,
